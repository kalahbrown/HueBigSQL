...
<%dashboard:layout_toolbar>
  ...
  <%def name="widgets()">
    ...
    <div data-bind="css: { 'draggable-widget': true },
        ...
        <div data-bind="css: { 'draggable-widget': true },
                    draggable: {data: draggableBigSQLAction(), isEnabled: true,
                    options: {'refreshPositions': true, 'stop': function(){ $root.isDragging(false); }, 'start': function(event, ui){ $root.isDragging(true); $root.currentlyDraggedWidget(draggableBigSQLAction());}}}"
         title="${_('BigSQL Script')}" rel="tooltip" data-placement="top">
         <a class="draggable-icon"><img src="${ static('oozie/art/bigsql.ico') }" class="app-icon"></a>
    </div>
