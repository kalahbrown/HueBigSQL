...
class Node():
  ...
  def to_xml(self, mapping=None, node_mapping=None, workflow_mapping=None):
    ...
    if self.data['type'] == 'bigsql' and not self.data['properties']['jdbc_bigsql_url']:
      self.data['properties']['jdbc_bigsql_url'] = _get_bigsql_url()
...
def _get_bigsql_url():
  try:
    return 'jdbc:db2://%s:%s/bigsql' % ('bi-mgmt01.corp.bigfishgames.com', '51000')
  except Exception, e:
    # Might fail ??
    LOG.warn('Could not guess bigsql URL: %s' % smart_str(e))
    return 'jdbc:db2://localhost:51000/bigsql'

class BigSQLAction(Action):
  TYPE = 'bigsql'
  FIELDS = {
     'script_path': {
          'name': 'script_path',
     ...
     # Common
     'jdbc_bigsql_url': {
          'name': 'jdbc_bigsql_url',
          'label': _('BigSQL URL'),
          'value': "",
          'help_text': _('e.g. jdbc:db2://<server>:51000/bigsql. JDBC URL for Big SQL.'),
          'type': ''
     },
     'password': {
          'name': 'password',
          ...
          'value': '',
          'help_text': _('The password element must contain the password of the current user. However, the password is only used if Hive Server 2 is backed by '
                         'something requiring a password (e.g. LDAP); non-secured Big SQL or Kerberized Big SQL don\'t require a password.'),
          'type': ''
     },
     'files': {
          'name': 'files',
